...
# ── Tests: close_ticket.py T082 — _git_root_for polish ───────────────────────

class TestGitRootForPolish:
    """T082: _git_root_for exposes git stderr; _git_stage WARNINGs include git root."""
    ...
    # (a) non-git path → _git_root_for returns non-empty stderr
    def test_non_git_path_returns_none_with_stderr(self, tmp_path):
        """_git_root_for on a plain directory returns (None, non-empty stderr string)."""
        plain = tmp_path / "not_a_repo"
        plain.mkdir()
        root, stderr = self.ct._git_root_for(plain / "file.txt")
        assert root is None
        assert stderr, f"Expected non-empty stderr for non-git path, got: {stderr!r}"

    def test_git_path_returns_root_with_empty_stderr(self, tmp_path):
        """_git_root_for inside a real git repo returns (root, '') with no stderr."""
        self._git_repo(tmp_path)
        (tmp_path / "file.txt").write_text("x")
        root, stderr = self.ct._git_root_for(tmp_path / "file.txt")
        assert root is not None
        assert stderr == ""

    # (a) WARNING includes git's stderr when git_root is None
    def test_git_stage_warning_includes_git_stderr(self, tmp_path, capsys):
        """_git_stage WARNING for a non-git dest includes git's actual error message."""
        plain = tmp_path / "not_a_repo"
        (plain / "archive").mkdir(parents=True)
        ticket = plain / "T999.md"
        ticket.write_text("ticket")
        dest = plain / "archive" / "T999.md"
        dest.write_text("archived")

        with pytest.raises(SystemExit) as exc_info:
            self.ct._git_stage(ticket, dest, None)

        assert exc_info.value.code == 2
        captured = capsys.readouterr()
        # git's stderr for a non-repo typically contains "not a git repository" or "fatal"
        assert any(
            phrase in captured.err.lower()
            for phrase in ("not a git", "fatal", "git error")
        ), f"Expected git error detail in WARNING:\n{captured.err}"

    # (b) WARNING includes the resolved git root when staging commands fail
    def test_git_stage_warning_includes_git_root_on_failure(self, tmp_path, capsys):
        """_git_stage WARNING after check_call failure includes git -C <git_root>."""
        from unittest.mock import patch

        self._git_repo(tmp_path)
        dest = tmp_path / "archive" / "T999.md"
        dest.parent.mkdir()
        dest.write_text("archived")
        ticket = tmp_path / "T999.md"
        ticket.write_text("ticket")

        with patch("close_ticket.subprocess.check_call",
        ...
        assert exc_info.value.code == 2
        captured = capsys.readouterr()
        assert f"-C {tmp_path}" in captured.err, (
            f"Expected 'git -C {tmp_path}' in staging instructions:\n{captured.err}"
        )
