...
def _setup(tmp_path: Path) -> Path:
    """Minimal harness tree. Returns the open ticket path."""
    docs = tmp_path / "docs"
...
def _run(tmp_root: Path, *args: str) -> subprocess.CompletedProcess:
    import os
    return subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "tools" / "close_ticket.py"), *args],
        capture_output=True, text=True,
        env={**os.environ, "HARNESS_ROOT": str(tmp_root), "PYTHONPATH": str(ROOT)},
    )
...
class TestPathOnly:
    ...
    def test_path_only_prints_ticket_path(self, tmp_path):
        ...
        result = _run(tmp_path, "T999", "--path-only")
    ...
    def test_path_only_not_found_exits_nonzero(self, tmp_path):
        ...
        _setup(tmp_path)
        result = _run(tmp_path, "T000", "--path-only")
    ...
    def test_path_only_exclusive_with_resolution(self, tmp_path):
        ...
        _setup(tmp_path)
        result = _run(tmp_path, "T999", "--path-only", "--resolution", "done")
        assert result.returncode != 0, \
