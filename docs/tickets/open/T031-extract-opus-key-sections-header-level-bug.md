...
- [ ] **Bug A:** regex changed to match either heading level — e.g.
      `re.compile(r"^#{1,2} Opus Review", re.MULTILINE)`. Verified against
      both the harness-root and a workspace `opus_notes.md`.
- [ ] **Bug B:** error message at line 77 (and any sibling error path) uses
      `path` rather than `OPUS_NOTES`.
- [ ] **Bug C:** `add_help=False` removed, OR a `--help` action added
      explicitly. `--help` exits 0 with usage text.
- [ ] Test in `tests/` (extend `test_workspace_path_flags.py` or a new file):
