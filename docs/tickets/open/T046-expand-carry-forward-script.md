...
- [ ] `scripts/tools/expand_carry_forward.py <finding-id>` accepts IDs in flexible
      formats: `S1#3`, `S1 #3`, `S3#3`, `s1#3` (case-insensitive).
- [ ] Searches all `opus_notes*.md` files — current (`docs/opus_notes.md`) and all
      files in `docs/archive/` matching `opus_notes*.md`.
- [ ] Extracts: the matching finding heading line + all text up to the next numbered
      finding heading (or end of section).
- [ ] If found in multiple files (carry-forward repeated), prints each occurrence with
      a source header `[From: <filename>]` and the session it appeared in.
- [ ] Exits 1 with a clear message if the ID is not found in any file.
- [ ] `--latest` flag: prints only the most recent occurrence (by session date in the
      file's `# Opus Review — S<N>` header).
- [ ] Test: synthetic `opus_notes.md` + archive file; assert correct extraction,
      multi-file output, not-found exit code.
- [ ] `extract_opus_key_sections.py --with-carry-forwards` output updated to include a
