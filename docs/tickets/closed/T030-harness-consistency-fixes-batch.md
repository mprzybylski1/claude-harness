...
- [x] `check_test_syntax` returns `SKIP` (not `PASS`) when zero test files found
- [x] Same for `check_utcnow` when none of the searched directories exist
- [x] `harness_config.py` docstring updated to current check names
- [x] `archive_session_log.py` docstring updated to match silent-success behavior
- [x] `prepare_opus_context.py` warns to stderr when `--opus PATH` doesn't exist
- [x] Test added to `test_workspace_gitignore.py` for same-repo skip branch
- [x] `session-close/SKILL.md` updated once T027 is resolved (done in T027)
...
S6 2026-05-25: All six sub-items fixed. (a) `check_test_syntax` returns SKIP when tests/ absent or empty; (b) session-close SKILL.md updated to pass `--repo` for workspace sessions (done in T027); (c) `harness_config.py` docstring corrected to current check names; (d) `archive_session_log.py` docstring corrected to silent-success behavior; (e) `prepare_opus_context.py` warns to stderr for missing --opus path; (f) `test_workspace_gitignore.py` same-repo skip branch test added.
