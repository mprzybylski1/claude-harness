...
## Resolution
Decision: option (c) — acceptable for a personal single-checkout harness. The hardcoded absolute paths in settings.json are intentional. They work correctly from workspace cwds (the T039 scenario) because Claude Code resolves hook commands from the settings file location, not the process cwd. If the repo is ever cloned to a different path, settings.json hook commands must be updated manually: replace /home/marcin/PycharmProjects/claude-harness/ with the new absolute path throughout. No code change made; decision recorded here.
