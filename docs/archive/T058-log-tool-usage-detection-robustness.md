...
- [x] `_candidate_paths` (or its callers) expands `~/` via `Path(path).expanduser()` before passing to `is_within_workspace`. Path-with-tilde Bash commands match their workspace. (Confirmed already handled by `is_within_workspace.expanduser().resolve()`; test added to lock this in.)
- [x] `_detect_workspace` inner-except calls `_log_error` (subject to T059 rate-limit when that lands) instead of silent `continue`.
- [x] `=`-value extractor in `_candidate_paths` uses `rsplit("=", 1)` so the right-hand path survives chained `=` tokens; OR document the limitation and accept it.
- [x] New tests: `--sessions=/path/file` extracted correctly, `KEY=val=/path` extracted correctly, `_detect_workspace` with cfg that raises logs and continues, `test_record_includes_workspace_field` covers harness-root case.
...
Fixed _detect_workspace inner-except to log error via _log_error instead of silent continue. Fixed _candidate_paths to use rsplit('=',1) so KEY=val=/path extracts /path correctly. ~/expansion confirmed already handled by is_within_workspace.expanduser().resolve() — no code change needed; test added to lock this in. Added 4 tests: chained-equals extraction, tilde expansion, inner-except logging, e2e harness-root workspace field.
