...
def _write_initial_files(ws_dir: Path, docs_dir: Path, name: str) -> None:
    ...
    (docs_dir / "sessions.md").write_text(
        ...
        "## Active Work\n\n**S0 — workspace created, no sessions yet**\n\n---\n\n"
        f"## Session Log\n\nS0 {today}: workspace created\n",
        encoding="utf-8",
    )
    ...
    (docs_dir / "tickets" / "INDEX.md").write_text(
        "# Ticket Index\n\n"
        f"Generated S0 {today}. 0 open tickets.\n"
...
def cmd_create(args: argparse.Namespace) -> None:
    ...
    while True:
        ...
        if not repo_path_str:
            continue
        if is_machine_specific_path(repo_path_str):
            print(
                f"  WARNING: '{repo_path_str}' uses a machine-specific absolute path. "
                f"Consider using ~/... instead so the workspace is portable."
            )
        ...
        repos.append({"name": repo_name, "path": portable_path(repo_path), "role": role})
    ...
    if docs_path_str:
        if is_machine_specific_path(docs_path_str):
            print(
                f"  WARNING: '{docs_path_str}' uses a machine-specific absolute path. "
                f"Consider using ~/... instead so the workspace is portable."
            )
    ...
    if docs_path_resolved:
        cfg["docs_path"] = portable_path(docs_path_resolved)
    if client_remote:
